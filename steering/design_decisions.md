...
## D-020: 新着検出を「時間窓」から「未配信差分」へ(--since を廃止し --pending を採用)

**日付**: 2026-08-03
**判断**: 日次バッチの新着検出を `--since <last_processed_iso>` による時間窓方式から、
`--pending`(`data/talks/*.json` の配信済み集合との差分)方式へ切り替える。

**背景(2026-08-03 の全期間監査)**:

2026-04-21〜2026-08-03 の 105 日を実測したところ、配信記録が実態と乖離していた。

| 指標 | 値 |
|---|---|
| 配信済み talk | 24 日 |
| `skipped_dates` | 66 日 |
| うち `daily:` commit がある(実際にバッチが走った) | 28 日 |
| うち commit が無い(後日 backfill された) | **38 日** |
| `daily:` commit が全く無い日 | **69 / 105 日** |
| delivered でも skipped でもない記録の穴 | 15 日 |

つまり **Scheduled Agent は実際には約 1/3 の日しか起動しておらず**、残りは後日の
backfill で一括して "no new TED-Ed upload" として埋められていた。

**恒久欠落は 0 件**(公開された 23 本すべて配信済み)だったが、backfill が在庫を
確認せず一律 skip 扱いしていたため、**実際には未配信在庫があった 7 日**が
虚偽の "no new release" として記録されていた:
2026-05-27 / 05-28 / 05-29 / 06-10 / 06-12 / 06-13 / 07-09(配信は 1〜4 日遅延)。

**時間窓方式の構造的欠陥**:

- `--since talks[0].published_at` は「前回配信した talk より新しいもの」しか見ない。
  バッチが起動しなかった日の在庫は、次回起動時にも窓の外に落ちたままになりうる。
- ted.com の `topic.videos` は 1 ページ 20 件で打ち切られ、旧実装はページングして
  いなかった(`--first 30` を渡しても 20 件しか返らない)。
- 既定の並び順は publishedAt 降順に見えるが厳密ではない(実測で 2026-04-16 が
  2026-04-14 の後に出現)。順序に依存した実装は危険。

**採用した対策**:

1. `scripts/fetch_ted_ed_talks.py --pending`
   - 配信済み video_id 集合(D-019 以前の YouTube ID 分はタイトルで補完)との差分で在庫判定
   - `after` カーソルでページングし 20 件上限を回避
   - `publishedAt` 昇順で返し、バッチは先頭 1 本を処理
   - 下限は「配信済み talk の最古 published_at」= 運用開始前の旧作を在庫扱いしない
   - **何日停止しても次回起動時に自動で追いつく(self-healing)**
2. `scripts/audit_delivery.py`
   - `pending` / `false-skip` / `uncovered` / `inconsistent` を検出し非ゼロ終了
   - `--since 2026-08-04` で D-020 以前の既知の歪みを除外し再発のみ監視
3. `prompts/daily_batch.md`
   - Step 3 を `--pending` に変更、Step 8.5(commit 前監査)と Step 9.6(backfill 前の在庫確認)を追加
   - 「在庫が 1 本でもある日は skip 記録してはならない」を明文化
4. `.github/workflows/agent-watchdog.yml`
   - 「36h 完全沈黙」判定に加え、監査の非ゼロ終了でも Issue を起票
   - 1 日単位の発火漏れ・誤 skip をその日のうちに検知できる

**過去データの扱い**: 既に配信済みで恒久欠落は無いため、過去の `skipped_dates` は
書き換えない(履歴の改変を避ける)。監査は `--since 2026-08-04` で再発のみを見る。

**残る限界**: Scheduled Agent 自体の発火漏れは本リポジトリからは制御できない。
`--pending` は「漏れても翌日以降に自動で取り戻す」ための緩和策であり、
発火率そのものを上げるものではない。watchdog Issue が継続的に立つ場合は
Agent 側の設定・クォータを疑うこと。
