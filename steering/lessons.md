...
## 2026-08-03 「毎日 skip」は実態ではなく記録の歪みだった(D-020)

**問題**: 「こんなに新作が無い日が続くのはおかしい」という指摘を受けて全期間を
監査したところ、`skipped_dates` の大半が **実際にバッチが走って新作が無かった日**
ではなく、**バッチが起動しなかった日を後日一括で埋めたもの**だと判明した。

2026-04-21〜2026-08-03(105日)の実測:

- `daily:` commit が存在する日は **36 / 105 日**(約 1/3)
- `skipped_dates` 66 日のうち **38 日は commit 無し**(後日 backfill)
- delivered でも skipped でもない「記録の穴」が 15 日

**幸い恒久欠落は 0 件**だった(TED-Ed が公開した 23 本すべて配信済み)。
TED-Ed 自体の公開頻度も実際に低く(105日で23本 ≒ 4.5日に1本)、
「新作が無い日が多い」こと自体は事実だった。

ただし backfill が在庫を確認せず一律 skip 扱いしていたため、
**未配信在庫があるのに "no new upload" と記録された日が 7 日**あった:
2026-05-27 / 05-28 / 05-29 / 06-10 / 06-12 / 06-13 / 07-09。
いずれも 1〜4 日遅れて配信されたが、サイト上には虚偽の "no new release" が残った。

**根本原因**:

1. `--since talks[0].published_at` の時間窓方式では、バッチが起動しなかった日の
   在庫が窓の外に落ちたままになりうる(取りこぼしが自動回復しない)。
2. `topic.videos` は 1 ページ 20 件で打ち切られるのに旧実装はページングしていなかった。
3. 既定の並び順は publishedAt 降順に見えて厳密ではない(2026-04-16 が 2026-04-14 の
   後に出現するケースを実測)。順序依存の実装は危険。
4. backfill 手順に「在庫確認」が無く、欠落日を機械的に skip として埋めていた。
5. watchdog が「36h 完全沈黙」でしか発火せず、1 日単位の発火漏れを検知できなかった。

**対応**(詳細は D-020):

- `fetch_ted_ed_talks.py --pending` — 時間窓ではなく配信済み集合との差分で在庫判定。
  ページング対応。何日止まっても次回起動時に自動で追いつく(self-healing)。
- `audit_delivery.py` — `pending` / `false-skip` / `uncovered` / `inconsistent` を検出。
- `daily_batch.md` Step 3 を `--pending` に変更、Step 8.5(commit 前監査)と
  Step 9.6(backfill 前の在庫確認)を追加。
- watchdog に監査ステップを追加し、非ゼロ終了でも Issue を起票。

**教訓**:

- **`skipped_dates` は「観測結果」であって「バッチが走った証拠」ではない**。
  記録を残す仕組みと、その記録が実際の観測に基づく保証は別物。
  「skip と記録されている」ことを「確認済み」と読み替えてはいけない。
- **時間窓ベースの差分検出は、実行が保証されない環境では必ず取りこぼす**。
  「前回からの差分」ではなく「あるべき状態との差分」で判定すれば自己修復する。
  cron の信頼性を上げるより、落ちても復帰する設計にする方が確実。
- **後処理の backfill は、元の観測を復元しない**。欠落日を埋めるときは
  「その日どうすべきだったか」を実データで確認してから埋めること。
- 異常に気付いたきっかけは人間の「そんなに無いことある?」という違和感だった。
  監視は数値の連続性(commit の有無・在庫の滞留)を機械的に見るべきで、
  今回 `audit_delivery.py` を watchdog に組み込んだのはそのため。
