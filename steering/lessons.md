...
### 追記(同日 Fable 5 による再レビューでの追加発見)

Opus 4.8 セッションの初回修正後、Fable 5 サブエージェントで再レビューしたところ
以下を追加で発見・修正した:

- **未エスケープ innerHTML(High)**: AI 生成の自由記述フィールド(意味・背景・要約・
  タイトル等)を `innerHTML` へ生挿入していたため、比較記号 `<` を含む文字列で
  `<` 以降が未閉じタグ扱いになり消えていた(実例: 2026-05-13 の `toddlers` の context
  `infant<toddler<preschooler` が「infant(乳児)」までしか表示されない)。`esc()`(&<> を
  エスケープ)を追加し全描画箇所へ適用。例文 `example.en` のみ仕様上 `<b>` 強調を使うため
  `escB()`(<b> だけ復元)で対応。**教訓: データ由来でも innerHTML には必ずエスケープ。
  英語本文と違い日本語解説フィールドには不等号・アンパサンドが普通に入る。**
- **統計カードのハードコード(Medium)**: カレンダー画面の「Current streak / This month /
  Total talks」が静的な "8" のままで、実データと無関係の虚偽値を常時表示していた。
  `renderCalendar()` で `calcStreak()` と実 talk 数から動的に更新するよう修正。
- **SAMPLE デモ talk の本番混入(要判断)**: `tryLoadRealData` が SAMPLE_TALKS を実データに
  マージするため、index.json に存在しないデモ5本(octopus/peek-a-boo 等)が timeline・
  streak に混入。製品判断のためユーザーに確認予定。
